...
const modalRequest = document.querySelector(".requestbutton")
const masks = document.querySelector('#modals-page-mask');

const closesButton = document.querySelector('.close-modal-button');
const closeButtons = document.querySelector('.closes-modal-button');
const modals=  document.querySelector('.requests-modal');
...
modalRequest.addEventListener('click', () => {
    // if (profile) {
        modals.style.opacity = "1"; //Makes the modal visible
        modals.style["pointer-events"] = "auto"; //Allow the user to click around in the modal
        masks.style.display = "block"; //Make the mask visible. This will darken the background because the mask's background color is black
        modalRequest.style["display"] = "none"; //Remove the button to create a modal since there is already a modal created
    // } else {
    //     alert("please sign in");
    //     console.log("hh");
    // }
