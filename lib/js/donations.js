...
let hahatable = document.querySelector(".hahatable");  
const thisobj = document.querySelector(".thisobj")
let  tdclass = document.querySelector(".tdclass")
let  trclass = document.querySelector(".trclass")
...
database.on('child_added', (data) => {
    ...
    console.log(data);
    if (data.key == ID) {
        console.log(data.val().requestDescription);
...
function displayComment(row) {      
    ...
    Object.keys(row.comments).forEach(key => {
        ...
        let commentNum  = row.comments[key].Number;
        ...
        content.innerHTML = commentNum+ " "+commentContent;
 ...
 function setTable(row)
 {
    let x = 10; 
        Object.keys(row.comments).forEach(key => {
            x = x-commentNum;
            tdclass.InnerHTML = row.comments[key].Comment;

        }); 
        console.log(x)
       
console.log(tdclass.InnerHTML)
...
function getComment() {
    ...
    let donoNumber = document.querySelector('#quantity-input')
    let stuffing = commentInput.value;
    let dono = donoNumber.value;
